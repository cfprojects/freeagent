<cfcomponent displayname="FreeAgent.IoC.IoCFactory" output="false" 
hint="I'm a generic IoC factory with simple init-method autowiring">
	...
	<cffunction name="init" access="public" output="false">
		...
		<cfif isSimpleValue(arguments.storage)>
			<cfif not len(trim(arguments.storage))>
				<cfset arguments.storage = "simplestorage" />
			</cfif>
	...
	<cffunction name="getPackage" access="private" output="false">
	...
	<cffunction name="getCascade" access="private" output="false">
		<cfreturn instance.cascade />
	</cffunction>
	...
	<cffunction name="containsBean" access="public" output="false" returntype="boolean">
		<cfargument name="beanName" type="string" required="true" />
		<cfreturn iif(hasDefinition(beanName) or cfcExists(instance.package & beanName),true,false) />
	</cffunction>
	...
	<cffunction name="getCachedBean" access="private" output="false">
		...
		<cfelse>
			<cfset obj = getStorage().fetch(beanName) />
	...
	<cffunction name="raiseMissingBeanException" access="private" output="false">
		<cfargument name="beanname" type="string" required="true" />
		<cfthrow type="FreeAgent.IoC.MissingBean" message="FreeAgent: Unable to locate bean." detail="#beanname#" />
	</cffunction>
	
	<cffunction name="cfcExists" access="private" output="false" returntype="boolean">
		<cfargument name="className" type="string" required="true" />
		<cfreturn fileExists("/" & listchangedelims(arguments.className,"/",".") & ".cfc") />
	</cffunction>
	...
	hint="indicates alternate IoC containers that can supply the requested bean for autoinit">
		<cfargument name="beanName" type="string" required="true" />
		<cfset var cascade = getCascade() />
		<cfif not len(trim(cascade))><cfreturn "" /></cfif>
		<cfreturn getFreeAgent().findBean(beanName,cascade) />
	</cffunction>
	...
	<cffunction name="constructBean" access="private" output="false" returntype="any">
		...
		<cfset var IoCManager = getFreeAgent() />
				...
				<cfif not structKeyExists(arg[x],"default") and structKeyExists(arg[x],"required") and arg[x].required>
					...
					not listfindnocase("string,numeric,date,boolean,struct,array,query,binary,xml,xmlelem,xmldoc",arg[x].type)>
						...
						<cfelse>
							...
							<cfif len(trim(obj))>
								<cfset obj = IoCManager.getBean(arg[x].name,obj) />
							</cfif>
						</cfif>
